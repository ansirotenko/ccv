    ...
    return (
        <div className="p-inputgroup flex-1">
            ...
            <span className={`p-inputgroup ${styles.searchGroup}`}>
                <InputText placeholder="search" value={inputValue} onChange={(e) => {
                    setInputValue(e.target.value);
                    onInputValueChange(e.target.value);
                }} />
                {inputValue && <i className={`pi pi-trash ${styles.clearSearch}`} onClick={() => {
                    setInputValue('');
                    if (onChange != null) {
                        onChange(null);
                    }
            ...
            <Button className={`pi pi-cog ${styles.buttons}`} onClick={() => {
                if (onSettings != null) {
                    onSettings();
                }
            }} />
            <Button className={`pi pi-times ${styles.buttons}`} onClick={() => {
                if (onClose != null) {
                    onClose();
                }
            }} />

...
impl Repository for InMemoryRepository {
    fn search(
        ...
        let page_size =
            usize::try_from(page_size).map_err(|_| app_error!("Page size cannot be negative"))?;
        if page_size <= 0 {
    ...
    fn remove(&self, item_ids: &Vec<&str>) -> Result<usize, AppError> {
        ...
        let old_length = items.len();
        items.retain(|c| !item_ids.contains(&c.id.as_str()));
        Ok(old_length - items.len())
    }
...
mod tests {
    ...
    fn insert_happy_path() {
        ...
        assert_eq!(
            items.get(2).unwrap().created,
            items.get(2).unwrap().last_reuse
        );
    ...
    fn update_last_resue_happy_path() {
        ...
        assert_eq!(
            items,
            vec![copy_item_unknown("1", t, new_last_reuse), item2]
        );
